...
public class bm20113 extends Ole2Adapter {
	...
	private    	String execlfilename = "bm20113.xls";  //excel檔名
	...
	public String[][] getDataValue(String[] wherestring)throws Exception{
		...
		ls_sql += " SELECT  MBCASEAPP_D.CASEID , ITEMID, INFORM, CANCEL, APP_LIC_DATE, RCV_LIC_DATE, COMPLETE_DATE, DELAY, HIGHLIGHT, ";
		ls_sql += " HLTYPE, MBCASEAPP_D.LICENSE_DESC , DIST, OWNER, O_TEL, CONTRATOR, C_TEL, CONTACT, CC_TEL, SITE_DIRECTOR, ";
		ls_sql += " SD_TEL, ADDRESS, SUPERVISOR, S_TEL, AGENT, A_TEL, DIST_CODE, HIGH_CODE  ";
		ls_sql += " FROM  MBCASEAPP_D,MBSTEP";
		ls_sql += " WHERE ((MBCASEAPP_D.CASEID = MBSTEP.CASEID) AND (MBCASEAPP_D.LICENSE_DESC = MBSTEP.LICENSE_DESC)) " ;


		if( !StringUtils.isEmpty(wherestring[0]) )
			ls_sql += " AND MBCASEAPP_D.CASEID = '" + wherestring[0] + "'";

		if( !StringUtils.isEmpty(wherestring[1]) )
			ls_sql += " AND MBCASEAPP_D.DIST_CODE = '" + wherestring[1] + "'";

		if( !StringUtils.isEmpty(wherestring[2]) && wherestring[2].equals("1"))
			ls_sql += " AND MBCASEAPP_D.HIGH_CODE = '1'";
		...
		if( !StringUtils.isEmpty(wherestring[3]) && wherestring[3].equals("3"))
			ls_sql += " AND MBSTEP.STEP IN ('2','3') ";
		else
			ls_sql += " AND MBSTEP.STEP = '" + wherestring[3] + "'";
		
		ls_sql += " GROUP BY MBCASEAPP_D.CASEID, ITEMID, INFORM, CANCEL, APP_LIC_DATE, RCV_LIC_DATE, COMPLETE_DATE, DELAY, HIGHLIGHT, HLTYPE, MBCASEAPP_D.LICENSE_DESC, DIST, OWNER, O_TEL, CONTRATOR, C_TEL, CONTACT, CC_TEL, SITE_DIRECTOR, SD_TEL, ADDRESS, SUPERVISOR, S_TEL, AGENT, A_TEL, DIST_CODE, HIGH_CODE "; 
		ls_sql += " ORDER BY DIST, HIGHLIGHT, MBCASEAPP_D.LICENSE_DESC ";
        ...
        System.err.println(ls_sql);
         ...
         while( rows1 != null && rows1.hasMoreElements() ){
			...
			String ls_where = "";
		    if(!StringUtils.isEmpty(wherestring[3]) && wherestring[3].equals("3")){
		   	   ls_where = " STEP IN ('2','3') AND CASEID ='" + wherestring[0] + "' AND LICENSE_DESC ='" + rds[i][3] + "'" ;
		    }else{
		   	   ls_where = " STEP =  '" + wherestring[3] + "' AND CASEID ='" + wherestring[0] + "' AND LICENSE_DESC ='" + rds[i][3] + "'" ;
		    }
           
            rds[i][4]   = Utils.convertToString(DBTools.dLookUp("MAX(CREDATE || ' ' || CRETIME)", "MBSTEP", ls_where, "SynctConn"));  
            rds[i][5]  = Utils.convertToString(row2.get("DIST"));
            rds[i][6]   = Utils.convertToString(row2.get("OWNER"));
            rds[i][7]   = Utils.convertToString(row2.get("O_TEL"));
            rds[i][8]   = Utils.convertToString(row2.get("CONTRATOR"));
            rds[i][9]   = Utils.convertToString(row2.get("C_TEL"));
            rds[i][10]  = Utils.convertToString(row2.get("CONTACT"));
            rds[i][11]  = Utils.convertToString(row2.get("CC_TEL"));
            rds[i][12]  = Utils.convertToString(row2.get("SITE_DIRECTOR"));
            rds[i][13]  = Utils.convertToString(row2.get("SD_TEL"));
            rds[i][14]  = Utils.convertToString(row2.get("ADDRESS"));
            rds[i][15]  = Utils.convertToString(row2.get("SUPERVISOR"));
            rds[i][16]  = Utils.convertToString(row2.get("S_TEL"));
            rds[i][17]  = Utils.convertToString(row2.get("AGENT"));
            rds[i][18]  = Utils.convertToString(row2.get("A_TEL"));
	...
	private void printHeader(String[] wherestring) throws Exception{
		...
		if( !StringUtils.isEmpty(wherestring[3])){
	    	if (wherestring[3].equals("1")){
	    		STEP_DESC = " 第" + wherestring[3] + "階段：回報案件明細";
	    	}else if (wherestring[3].equals("2")){
	    		STEP_DESC = " 第" + wherestring[3] + "階段：回報案件明細";
	    	}else if (wherestring[3].equals("3")){
	    		STEP_DESC = " 第" + wherestring[3] + "階段：回報案件明細（包含第2階段回報資料）";
